...
const condition = {
  contains(value, array) {
    if (array.filter(item => item === value).length === 0)
      throw error.badRequest(
        `파라미터 처리 오류입니다. (${value})을(를) ${JSON.stringify(
          array
        )} 중 하나의 값을 입력해주세요.`
      );
  },
  ...
  array(value) {
    if (!Array.isArray(value))
      throw error.badRequest(
        `파라미터 처리 오류입니다. 배열만 입력 가능합니다(${value}).`
      );
  },
  arrays(value) {
    if (Array.isArray(value)) {
      value.map(item => {
        if (!Array.isArray(item))
          throw error.badRequest(
            `파라미터 처리 오류입니다. 배열만 입력 가능합니다(${item}).`
          );
      });
    } else {
      throw error.badRequest(
        `파라미터 처리 오류입니다. 잘못된 매개변수입니다(${value}).`
      );
    }
  },
  number(value, range) {
    if (typeof value !== 'number')
      throw error.badRequest(
        `파라미터 처리 오류입니다. 숫자만 입력 가능합니다(${value}).`
      );
    if (range.min !== undefined && range.min !== null && range.min > value)
      throw error.badRequest(
        `파라미터 처리 오류입니다. (${value}를 ${
          range.min
        }이상으로 입력해주세요).`
      );
  ...
  string(value, range) {
    if (typeof value !== 'string')
      throw error.badRequest(
        `파라미터 처리 오류입니다. 문자열만 입력 가능합니다(${value}).`
      );
    if (
      range.min !== undefined &&
      range.min !== null &&
      range.min > value.length
    )
      throw error.badRequest(
        `파라미터 처리 오류입니다. (${value}를 ${
          range.min
